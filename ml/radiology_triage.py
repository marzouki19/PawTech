...
def _load_clinical_photo_dataset(path: str) -> List[Dict[str, str]]:
...
def _normalize_organ_label(value: str) -> str:
...
def _severity_to_level(value: str) -> int:
    sev = _normalize_severity_label(value)
    if sev == "critical":
        return 3
    if sev == "high":
        return 2
...
def _augment_reference_images(img_rgb: Image.Image) -> List[Image.Image]:
    """
    Lightweight augmentation to make matching more robust with tiny datasets.
    """
    base = img_rgb.convert("RGB")
    # Keep augmentation conservative to avoid class drift on tiny datasets.
    return [
        base,
        ImageOps.mirror(base),
        ImageEnhance.Contrast(base).enhance(1.08),
    ]


def _extract_gray_signature_from_image(img_rgb: Image.Image, target_size: Tuple[int, int] = (96, 96)) -> np.ndarray:
    """
    Compact grayscale structural signature (for shape/texture similarity).
    """
    gray = np.asarray(img_rgb.convert("L").resize(target_size), dtype=np.float32)
    flat = gray.reshape(-1)
...
def _eye_hint_confidence(path: str) -> float:
    """
    Lightweight heuristic to detect likely eye photos:
    - significant pink/red tissue around
    - dark compact region (pupil/cornea) in center area
    """
    img = Image.open(path).convert("RGB").resize((160, 160))
    arr = np.asarray(img, dtype=np.float32)
    r = arr[:, :, 0]
    ...
    # Red/pink inflamed tissue proxy
    red_mask = (r > 120) & (r > g * 1.08) & (r > b * 1.05)
    red_ratio = float(np.mean(red_mask))

    gray = np.mean(arr, axis=2)
    h, w = gray.shape
    center = gray[h // 4 : (3 * h) // 4, w // 4 : (3 * w) // 4]
    dark_center_ratio = float(np.mean(center < 70))

    # Eye-like pattern is not only redness; it needs a compact dark center.
    red_term = min(1.0, red_ratio / 0.22)
    dark_term = min(1.0, dark_center_ratio / 0.28)
    balanced_gate = 1.0
    if dark_center_ratio < 0.06:
        balanced_gate *= 0.55
    if dark_center_ratio > 0.42:
        balanced_gate *= 0.65
    if red_ratio > 0.55:
        balanced_gate *= 0.75

    # Smooth confidence [0..1], but gated to avoid wound->eye confusion.
    score = ((0.50 * red_term) + (0.50 * dark_term)) * balanced_gate
    return float(max(0.0, min(1.0, round(score, 3))))


def _heart_hint_confidence(path: str) -> float:
    """
    Heart-photo hint:
    - dominant dark red tissue
    - moderate texture, low central black-core signal
    """
    img = Image.open(path).convert("RGB").resize((160, 160))
    arr = np.asarray(img, dtype=np.float32)
    r = arr[:, :, 0]
    ...
    red_dom = float(np.mean((r > 90) & (r > g * 1.12) & (r > b * 1.12)))
    dark_core = float(np.mean(gray < 55))
    texture = float((np.abs(np.diff(gray, axis=0)).mean() + np.abs(np.diff(gray, axis=1)).mean()) / 2.0)
    texture_norm = min(1.0, texture / 22.0)

    score = (0.60 * min(1.0, red_dom / 0.45)) + (0.25 * (1.0 - min(1.0, dark_core / 0.30))) + (0.15 * texture_norm)
    return float(max(0.0, min(1.0, round(score, 3))))


def _paw_hint_confidence(path: str) -> float:
    """
    Paw-photo hint:
    - dark compact pads + bright skin/hair contrast
    - mixed texture due to fur + paw pads
    """
    img = Image.open(path).convert("RGB").resize((160, 160))
    arr = np.asarray(img, dtype=np.float32)
    gray = np.mean(arr, axis=2)
    ...
    redness = float(np.mean((r > 110) & (r > g * 1.05) & (r > b * 1.03)))
    texture = float((np.abs(np.diff(gray, axis=0)).mean() + np.abs(np.diff(gray, axis=1)).mean()) / 2.0)
    texture_norm = min(1.0, texture / 26.0)

    score = (
        0.35 * min(1.0, dark_ratio / 0.22)
        + 0.25 * min(1.0, bright_ratio / 0.35)
        + 0.20 * min(1.0, redness / 0.18)
        + 0.20 * texture_norm
    )
    return float(max(0.0, min(1.0, round(score, 3))))


def _ear_hint_confidence(path: str) -> float:
    """
    Ear-photo hint:
    - pink/red tissue with elongated folds
    - less central dark compact region than eye
    """
    img = Image.open(path).convert("RGB").resize((160, 160))
    arr = np.asarray(img, dtype=np.float32)
    r = arr[:, :, 0]
    ...
    pink_red = float(np.mean((r > 105) & (r > g * 1.06) & (r > b * 1.03)))
    h, w = gray.shape
    center = gray[h // 4 : (3 * h) // 4, w // 4 : (3 * w) // 4]
    dark_center = float(np.mean(center < 70))
    gx = np.abs(np.diff(gray, axis=1))
    gy = np.abs(np.diff(gray, axis=0))
    anisotropy = float(abs(gx.mean() - gy.mean()) / max(1.0, gx.mean() + gy.mean()))

    score = (0.55 * min(1.0, pink_red / 0.30)) + (0.25 * (1.0 - min(1.0, dark_center / 0.24))) + (0.20 * min(1.0, anisotropy / 0.22))
    return float(max(0.0, min(1.0, round(score, 3))))


def _classify_clinical_photo(path: str, dataset_path: str) -> Dict[str, Any]:
    rows = _load_clinical_photo_dataset(dataset_path)
    # Merge with radiology annotations as extra labeled references when available.
    radiology_rows = _load_clinical_photo_dataset(os.path.join("ml", "data", "radiology_annotated_dataset.csv"))
    if radiology_rows:
        rows.extend(radiology_rows)
    if not rows:
        return {"success": False, "error": "No clinical photo dataset found."}

    project_dir = os.getcwd()
    x_query = _extract_clinical_photo_features(path)
    query_img = Image.open(path).convert("RGB")
    g_query = _extract_gray_signature_from_image(query_img)

    # Keep class samples for generic multi-organ matching.
    class_samples: Dict[str, List[np.ndarray]] = {}
    class_severity_samples: Dict[str, List[str]] = {}
    sample_candidates: List[Tuple[str, str, float]] = []
    class_counts: Dict[str, int] = {}

    for row in rows:
        img_rel = str(row.get("image_path", "")).strip()
        organ = _normalize_organ_label(str(row.get("organ_label", "")))
        severity = _normalize_severity_label(str(row.get("severity_label", "low")))
        if img_rel == "" or organ == "":
            continue
        img_abs = img_rel if os.path.isabs(img_rel) else os.path.join(project_dir, img_rel)
        if not os.path.exists(img_abs):
            continue
        try:
            img_ref = Image.open(img_abs).convert("RGB")
            for aug in _augment_reference_images(img_ref):
                x_ref = _extract_clinical_photo_features_from_image(aug)
                g_ref = _extract_gray_signature_from_image(aug)
                # cosine distance on normalized vectors
                dist_feat = float(1.0 - np.dot(x_query, x_ref))
                dist_gray = float(1.0 - np.dot(g_query, g_ref))
                dist = (0.72 * dist_feat) + (0.28 * dist_gray)
                sample_candidates.append((organ, severity, dist))
                class_samples.setdefault(organ, []).append(x_ref)
                class_severity_samples.setdefault(organ, []).append(severity)
                class_counts[organ] = class_counts.get(organ, 0) + 1
        except Exception:
            continue

    if not sample_candidates or not class_samples:
        return {"success": False, "error": "No valid clinical photo samples."}

    # Stable generic approximation:
    # 1) pick organ by hybrid distance per class (nearest + short-k average)
    # 2) pick severity from neighbors of selected organ only
    class_dist_map: Dict[str, List[float]] = {}
    class_sev_pairs: Dict[str, List[Tuple[str, float]]] = {}
    for organ, sev, dist in sample_candidates:
        class_dist_map.setdefault(organ, []).append(float(dist))
        class_sev_pairs.setdefault(organ, []).append((sev, float(dist)))

    class_scores: List[Tuple[str, float]] = []
    for organ, dists_raw in class_dist_map.items():
        dists = sorted(dists_raw)
        k = min(4, len(dists))
        nearest = dists[0]
        knn_mean = float(np.mean(dists[:k]))
        # Strongly favor the most similar known sample (true approximation mode).
        score = (0.90 * nearest) + (0.10 * knn_mean)
        class_scores.append((organ, score))

    class_scores.sort(key=lambda x: x[1])
    best_organ, best_score = class_scores[0]
    second_score = class_scores[1][1] if len(class_scores) > 1 else (best_score + 0.25)

    # Confidence based on best class absolute distance + margin with second class.
    # lower distance => higher confidence.
    abs_conf = float(max(0.0, min(1.0, 1.0 - (best_score / 0.85))))
    margin = float(max(0.0, second_score - best_score))
    margin_conf = float(max(0.0, min(1.0, margin / 0.22)))
    confidence = round((0.65 * abs_conf) + (0.35 * margin_conf), 3)

    # Ambiguous-case tie-break using targeted hints (eye/ear/paw/heart).
    # Applied only when confidence is low or class gap is weak.
    hint_scores: Dict[str, float] = {}
    if "eye" in class_dist_map:
        hint_scores["eye"] = _eye_hint_confidence(path)
    if "ear" in class_dist_map:
        hint_scores["ear"] = _ear_hint_confidence(path)
    if "paw" in class_dist_map:
        hint_scores["paw"] = _paw_hint_confidence(path)
    if "heart" in class_dist_map:
        hint_scores["heart"] = _heart_hint_confidence(path)

    if hint_scores and (confidence < 0.62 or margin < 0.05):
        top_organs = {o for o, _ in class_scores[:3]}
        hinted_organ, hinted_score = sorted(hint_scores.items(), key=lambda x: x[1], reverse=True)[0]
        current_hint = float(hint_scores.get(best_organ, 0.0))
        if hinted_organ in top_organs and hinted_score >= 0.52 and (hinted_score - current_hint) >= 0.10:
            best_organ = hinted_organ
            confidence = round(max(confidence, 0.45 + (0.40 * hinted_score)), 3)

    # Severity from selected-organ neighbors only (prevents cross-organ severity leak).
    organ_neighbors = sorted(class_sev_pairs.get(best_organ, []), key=lambda t: t[1])
    k_sev = min(5, len(organ_neighbors))
    sev_w_sum = 0.0
    sev_total = 0.0
    for sev, dist in organ_neighbors[:k_sev]:
        w = 1.0 / max(1e-6, dist + 1e-6)
        sev_w_sum += w * _severity_to_level(sev)
        sev_total += w
    sev_level = (sev_w_sum / sev_total) if sev_total > 0 else 0.0
    best_severity = _level_to_severity(sev_level)

    # Severity calibration for clinical photos to reduce false high/critical.
    pathology_score = _clinical_pathology_score(path)
    best_level = _severity_to_level(best_severity)
    if confidence < 0.58 and best_level >= 2:
        best_level = 1  # downgrade to medium when organ match is uncertain
    if pathology_score < 0.26:
        best_level = 0
    elif pathology_score < 0.45 and best_level >= 2:
        best_level = 1
    # Critical is allowed only with very strong confidence + severe visual pattern.
    if best_level == 3 and not (confidence >= 0.82 and pathology_score >= 0.82):
        best_level = 2
    best_severity = _level_to_severity(float(best_level))

    # Keep best approximation even when confidence is low.
    # We still surface low confidence to the UI via organ_confidence.

    organ_candidates = []
    for organ, score in class_scores:
        cand_score = float(max(0.0, min(1.0, 1.0 - (score / 0.95))))
        organ_candidates.append({organ: round(cand_score, 3)})

    return {
        "success": True,
        "probable_organ": best_organ,
        "severity": best_severity if best_organ != "undetermined" else "low",
        "organ_confidence": confidence,
        "organ_candidates": organ_candidates,
        "hint_scores": {k: round(v, 3) for k, v in hint_scores.items()},
...
def _build_clinical_photo_response(
    photo_result: Dict[str, Any],
    metrics: Dict[str, float],
    is_radio_like: bool,
) -> Dict[str, Any]:
    severity = str(photo_result.get("severity", "low")).lower()
    recommendation = _consultation_recommendation(severity)
    organ = str(photo_result.get("probable_organ", "undetermined")).lower()
    confidence = float(photo_result.get("organ_confidence", 0.0))

    if organ == "undetermined":
        summary = "Clinical-photo mode: organ classification confidence is low."
        findings = [
            "Image analyzed in clinical-photo mode.",
            "Organ localization confidence is low; please provide a clearer photo or X-ray.",
        ]
    else:
        summary = (
            f"Clinical-photo mode detected probable organ: {organ}. "
            f"This is visual approximation support; validate clinically."
        )
        findings = [
            "Image analyzed in clinical-photo mode.",
            f"Nearest visual match suggests organ: {organ}.",
        ]
        if confidence < 0.52:
            findings.append(
                "Confidence is low for this approximation; use veterinary exam to confirm organ."
            )

    if is_radio_like:
        findings.append("Image also appeared radiography-like, but photo-matching confidence was prioritized.")

    return {
        "success": True,
        "severity": severity,
        "risk_score": round(20.0 + confidence * 50.0, 1),
        ...
        "organ_candidates": photo_result.get("organ_candidates", []),
        "urgent_consultation": recommendation["urgent_consultation"],
        "recommended_timing": recommendation["recommended_timing"],
        "next_step": recommendation["next_step"],
        "source": "clinical_photo_match_v2_hybrid",
        "input_metrics": metrics,
        "hint_scores": photo_result.get("hint_scores", {}),
    }
