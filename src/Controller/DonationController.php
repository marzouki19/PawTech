...
use App\Service\DonorRetentionService;
...
class DonationController extends AbstractController
{
    ...
    #[Route('/predict-retention', name: 'app_donation_predict_retention', methods: ['GET', 'POST'])]
    public function predictRetention(Request $request, DonorRetentionService $retentionService): Response
    {
        $prediction = null;
        $error = null;
        
        // Check if model is available
        if (!$retentionService->isModelAvailable()) {
            $error = 'ML model not available. Please train the model first.';
        } else {
            // Get all validated donations
            $donations = $this->repository->findBy(['statut' => true]);
            ...
            foreach ($donations as $donation) {
                ...
                if (!isset($donorData[$email])) {
                    $donorData[$email] = [
                        'email' => $email,
                        'donateur' => $donation->getDonateur(),
                        'donations' => [],
                        'lastDonation' => $donation->getDate()
                    ];
                }
                
                $donorData[$email]['donations'][] = $donation;
                if ($donation->getDate() > $donorData[$email]['lastDonation']) {
                    $donorData[$email]['lastDonation'] = $donation->getDate();
                }
            }
            
            // Calculate predictions for each donor
            $predictions = [];
            foreach ($donorData as $email => $data) {
                try {
                    $metrics = $retentionService->calculateDonorMetrics($data['donations'], $data['lastDonation']);
                    $result = $retentionService->predictRetention($metrics);
                    $predictions[] = [
                        'email' => $email,
                    ...
                    // Skip donors with errors
                }
            }
            
            // Sort by risk (high risk first)
            usort($predictions, function($a, $b) {
                return $b['prediction']['probability_wont_donate'] - $a['prediction']['probability_wont_donate'];
            });
            
            $prediction = $predictions;
        }
        
        return $this->render('donation/predict_retention.html.twig', [
            'active' => 'donations',
            'page_title' => 'Donor Retention Prediction',
            'prediction' => $prediction,
            'error' => $error,
            'model_available' => $retentionService->isModelAvailable()
        ]);
    }
}
