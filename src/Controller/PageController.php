...
final class PageController extends AbstractController
{
    ...
    #[Route('/signin', name: 'app_signin_page', methods: ['GET', 'POST'])]
    public function signin(Request $request, SupabaseUserRepository $userRepository, UserPasswordHasherInterface $passwordHasher): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($user) {
                ...
                if (! $userRepository->save($user)) {
                    $error = [
                        'messageKey' => 'Account creation failed. Please try again.',
                        'messageData' => [],
                    ];
                } else {
                    $roles = $user->getRoles();
    ...
    public function settings(Request $request, SupabaseUserRepository $userRepository): Response
    {
        $sessionUser = $request->getSession()->get('user');
        $userId = is_array($sessionUser) && isset($sessionUser['id']) ? (int) $sessionUser['id'] : null;

        if (!$userId) {
            return $this->redirectToRoute('app_signin_page');
        }
    ...
    {
        return $this->render('dogs.html.twig');
    }
    ...
    public function veterinarianPage(UserRepository $userRepository): Response
    {
        ...
        return $this->render('veterinarian.html.twig', [
