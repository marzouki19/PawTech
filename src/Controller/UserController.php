...
final class UserController extends AbstractController
{
        ...
        public function ajaxSort(Request $request, SupabaseUserRepository $userRepository): Response
        {
            ...
            usort($users, static function (User $left, User $right) use ($sortBy, $direction): int {
    ...
    public function new(Request $request, SupabaseUserRepository $userRepository, SluggerInterface $slugger, UserPasswordHasherInterface $passwordHasher): Response
    {
        $user = new User();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$userRepository->save($user)) {
                $this->addFlash('error', 'Unable to create user.');
    ...
    public function show(int $id, SupabaseUserRepository $userRepository): Response
    {
    ...
    public function edit(Request $request, SupabaseUserRepository $userRepository, SluggerInterface $slugger, UserPasswordHasherInterface $passwordHasher, int $id): Response
    {
        $user = $userRepository->find($id);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$userRepository->save($user)) {
                $this->addFlash('error', 'Unable to update user.');
    ...
    public function delete(Request $request, int $id, SupabaseUserRepository $userRepository): Response
    {
        $user = $userRepository->find($id);
   ...
   */
    ...
    public function signup(Request $request, SupabaseUserRepository $userRepository, UserPasswordHasherInterface $passwordHasher): Response
    {
        $errors = [];
        $data = [
            'prenom' => '',
    ...
    public function signin(Request $request, SupabaseUserRepository $userRepository, UserPasswordHasherInterface $passwordHasher, JwtTokenService $jwtTokenService): Response
    {
        ...
        if ($error === null && $lastEmail !== '') {
            ...
            if ($user) {
                if (!$passwordHasher->isPasswordValid($user, $password)) {
                    ...
                    return $jwtTokenService->attachAuthCookie(
    ...
    #[Route('/verify-face-proxy', name: 'app_verify_face_proxy', methods: ['POST'])]
    public function verifyFaceProxy(Request $request): JsonResponse
    {
        $data = json_decode($request->getContent(), true);
        $userFace = $data['user_face'] ?? null;

        // This is a stub implementation. Integrate with your face verification provider here.
        if (!$userFace) {
            return new JsonResponse(['message' => 'no'], Response::HTTP_BAD_REQUEST);
        }

        // For now, always return no-match. Update logic to verify against stored faces.
        return new JsonResponse(['message' => 'no']);
    }
 ...
 private function renderEntity(
    ...
    public function users(Request $request, SupabaseUserRepository $userRepository, SluggerInterface $slugger, UserPasswordHasherInterface $passwordHasher): Response
    {
        ...
        if ($searchQuery !== '') {
            $users = $userRepository->search($searchQuery, $searchField);
        } else {
